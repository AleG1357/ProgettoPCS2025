...
// Triangolazione Class I di un poliedro con parametro val (val > 0)
Polyhedron TriangleClassI(const Polyhedron& P_old, const unsigned int& val)
{
	Polyhedron P;
	P.id = P_old.id;
	if (val == 1) return P_old;
	...
	{
		unsigned int idA = face.idVertices[0];
		unsigned int idB = face.idVertices[1];
		unsigned int idC = face.idVertices[2];
		...
		for (unsigned int i = 0; i <= val; ++i)
			...
			{
				Vector3d coords = (i * A.coords + j * B.coords + (val - i - j) * C.coords) / val;
				unsigned int idV = addVertexIfMissing(P, coords);
				Vij[{i, j}] = idV;
			}
		for (unsigned int i = 0; i < val; ++i)
			for (unsigned int j = 0; j < val - i; ++j)
			{
				...
				unsigned int e0 = addEdgeIfMissing(P, id0, id1);
				unsigned int e1 = addEdgeIfMissing(P, id1, id2);
				unsigned int e2 = addEdgeIfMissing(P, id2, id0);
				Face f0;
				f0.id = static_cast<unsigned int>(P.faces.size());
				f0.idVertices = {id0, id1, id2};
				f0.idEdges = {e0, e1, e2};
				...
				if (i + j < val - 1)
				{
					...
					unsigned int e3 = addEdgeIfMissing(P, id1, id2);
					unsigned int e4 = addEdgeIfMissing(P, id2, id3);
					unsigned int e5 = addEdgeIfMissing(P, id3, id1);
					Face f1;
					f1.id = static_cast<unsigned int>(P.faces.size());
					f1.idVertices = {id1, id2, id3};
					f1.idEdges = {e3, e4, e5};
...
// Triangolazione Class II di un poliedro con parametro val (b = c = val > 0)
Polyhedron TriangleClassII(const Polyhedron& P_old, const unsigned int& val)
{
	...
	for (const auto& e : P_I.edges) {
		...
		unsigned int bc0 = face_barycenter[e.faceNeighbors[0]];
		unsigned int bc1 = face_barycenter[e.faceNeighbors[1]];
		unsigned int e0 = addEdgeIfMissing(P, v0, bc0);
		unsigned int e1 = addEdgeIfMissing(P, bc0, bc1);
		unsigned int e2 = addEdgeIfMissing(P, bc1, v0);
		Face f0;
		f0.id = P.faces.size();
		f0.idVertices = {v0, bc0, bc1};
		f0.idEdges = {e0, e1, e2};
		P.faces.push_back(f0);
		unsigned int e3 = addEdgeIfMissing(P, bc0, v1);
		unsigned int e4 = addEdgeIfMissing(P, v1, bc1);
		Face f1;
		f1.id = P.faces.size();
		f1.idVertices = {bc0, v1, bc1};
		f1.idEdges = {e3, e4, e1};
		P.faces.push_back(f1);
	}
	return P;
